...
void control_logic() {
    ...
    while(PC < INSTRUCTION_MEMORY_GLOBAL_LENGTH -1) {
        ...
        if (strcmp(instruction_header, "add") == 0) {
            ...
            printf("\ndebug: we are adding registers: %s and %s to register: %s", operand_A, operand_B,
                   destination_register);
            if (strcmp(INSTRUCTION_MEMORY_GLOBAL[PC][1], "add") == 0) {
                exec_add(INSTRUCTION_MEMORY_GLOBAL[PC][2], INSTRUCTION_MEMORY_GLOBAL[PC][3],
                         INSTRUCTION_MEMORY_GLOBAL[PC][4]);
                printf("\ndebug: we are adding registers: %s and %s to register: %s", INSTRUCTION_MEMORY_GLOBAL[PC][3],
                       INSTRUCTION_MEMORY_GLOBAL[PC][4], INSTRUCTION_MEMORY_GLOBAL[PC][2]);
                PC++;
            }
            else if (strcmp(INSTRUCTION_MEMORY_GLOBAL[PC][1], "addi") == 0) {
                exec_addi(INSTRUCTION_MEMORY_GLOBAL[PC][2], INSTRUCTION_MEMORY_GLOBAL[PC][3],
                          INSTRUCTION_MEMORY_GLOBAL[PC][4]);
                PC++;
            }
            else if (strcmp(INSTRUCTION_MEMORY_GLOBAL[PC][1], "sub") == 0) {
                exec_sub(INSTRUCTION_MEMORY_GLOBAL[PC][2], INSTRUCTION_MEMORY_GLOBAL[PC][3],
                         INSTRUCTION_MEMORY_GLOBAL[PC][4]);
                PC++;
            }
            else if (strcmp(INSTRUCTION_MEMORY_GLOBAL[PC][1], "slt") == 0) {
                exec_slt(INSTRUCTION_MEMORY_GLOBAL[PC][2], INSTRUCTION_MEMORY_GLOBAL[PC][3],
                         INSTRUCTION_MEMORY_GLOBAL[PC][4]);
                PC++;
            }
            else if (strcmp(INSTRUCTION_MEMORY_GLOBAL[PC][1], "sltu") == 0) {
                exec_sltu(INSTRUCTION_MEMORY_GLOBAL[PC][2], INSTRUCTION_MEMORY_GLOBAL[PC][3],
                          INSTRUCTION_MEMORY_GLOBAL[PC][4]);
                PC++;
            }

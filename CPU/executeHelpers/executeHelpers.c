...
void load_word(char* target_register_name, char* base_address_register_name, int offset){
    ...
    int retrieved_value = read_data_from_memory(memory_index + offset/4); // idk
...
void jump(char* jump_label) {
    PC_GLOBAL = find_label_index(jump_label);
}

void jump_and_link(char* jump_label) {
    char* $ra = REGISTER_STRING_NAMES[31];
    int return_address = PC_GLOBAL+1;
    write_data_to_register($ra, return_address);
    PC_GLOBAL = find_label_index(jump_label);
}

void jump_register() {
    char* $ra = "$ra";
    PC_GLOBAL = find_register_value($ra);
}
